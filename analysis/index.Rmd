...
plotDays <- 5*365 # How many days to show in plot (5 years)

bitcoin_2023 <- as.data.frame(bitcoin_2023[order(desc(bitcoin_2023$timestamp)),])

# mma_200 <- mean(as.vector(bitcoin_2023[1:nDays, "close"]))
# mayers_index <- bitcoin_2023$close[1]/mma_200
# 
# cat("Rigth now ", as.character(Sys.time()),"\n",
#     "the index is " = mayers_index, "\n", 
#     "the price is "= bitcoin_2023$close[1], "\n")
...
for( i in c(1:nRange)){
  ...
  if(mayers_index > 1 & mayers_index <= 1.5){index_summary$decision[i] <- "Wait to Buy";  index_summary$color[i] <- "#0000E2"}
...
# Create a named vector of colors corresponding to unique values
color_legend_labels <- c("Strong Buy" = "#69b3a2", 
                        "Wayte to buy" = "#0000E2", 
                        "Sell" = "#FF7F00",
                        "Strong Sell" = "#FF0000")
