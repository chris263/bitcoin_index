...
num_years = 4
# Make multiple requests to get historical data
for (i in 1:num_years) {
  start_time_request <- Sys.Date() - 365*i
  end_time_request <- Sys.Date() - 365*(i-1)

  res <- binance_klines('BTCBRL',
                         interval = '1d',
                         start_time = start_time_request,
                         end_time = end_time_request)
  
  # Extract and append the parsed content to the list
  historical_data <- rbind(historical_data, res)
}

historical_data$open_time <- as.Date(historical_data$open_time)
historical_data <- historical_data[order(-historical_data$open_time),]
library(dplyr)
historical_data <- historical_data[!duplicated(historical_data$open_time),]
...
rownames(crypto_data) <- as.character(historical_data$open_time)
...
cat(" Last date:", as.character(Sys.Date()), " Multiple Mayer's index is: ", round(index_summary[index_summary$date == Sys.Date(), "index"],2),"\n",
    "Price = ", format.money(bitcoin_2023[bitcoin_2023$timestamp == lastDate, "Close"]),"\n",
